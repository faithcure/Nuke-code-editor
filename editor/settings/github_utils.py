...
def _make_secure_https_url(repo_url, username, token):
    if not repo_url:
        raise ValueError("Repository URL is missing.")
    if not repo_url.startswith("https://"):
        raise ValueError("Only HTTPS GitHub URLs are supported for token authentication.")
...
def load_github_credentials(settings_file=None):
    """
...
def push_to_github(editor_window, settings_file=None):
    ...
    try:
        ...
        secure_url = _make_secure_https_url(repo_url, username, token)
        repo.git.push(secure_url, f"{current_branch.name}:{current_branch.name}")
...
def add_remote(editor_window, repo_name, settings_file=None):
    ...
    try:
        ...
        username, _token, repo_url = load_github_credentials(settings_file)
...
def pull_from_github(editor_window, settings_file=None):
    ...
    try:
        ...
        secure_url = _make_secure_https_url(repo_url, username, token)
        current_branch = repo.active_branch
        repo.git.pull(secure_url, current_branch.name)
        ...
        QMessageBox.critical(editor_window, "Pull Failed", str(e))
    except Exception as e:
        QMessageBox.critical(editor_window, "Unexpected Error", f"An unexpected error occurred: {e}")
